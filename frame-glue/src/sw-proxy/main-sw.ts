...
async function handleInitProxy(event: FetchEvent) {
...
export async function handleFetch(event: FetchEvent): Promise<Response> {
  ...
  const isInit =
    url.origin == self.origin && url.pathname == "/sw-api/init-proxied-sw"
  if (isInit) handleInitProxy(event)
